...
//         return 1.0f;  // Par défaut, on retourne 1.0 si erreur
//     }

//     std::string line;
//     // Ici, on échappe bien les guillemets avec \"
...
void sceneNiveau1(Scene *scene){
    ...
    // std::string path = "../animations/mario/Idle.dae";
    // float scaleFromFile = readDaeUnitScale(path);
    // //std::cout<<"UnitScale : "<<scaleFromFile<<std::endl;
    // //std::cout << "Chargement de la première animation..." << std::endl;
    // GOmariometal.animationMeshes.loadAnimatedModel("../animations/mario/Idle.dae", scaleFromFile*2);
    // GOmariometal.hasAnimation = true;
    // //std::cout << "Chargement des animations supplémentaires..." << std::endl;
    // GOmariometal.animationMeshes.addAnimation("../animations/mario/Walking.dae");
    // GOmariometal.animationMeshes.addAnimation("../animations/mario/Running.dae");
    // GOmariometal.animationMeshes.addAnimation("../animations/mario/Jump.dae");
    // GOmariometal.setAnimation("../animations/mario/Idle.dae");

    // for(auto& mesh : GOmariometal.animationMeshes.baseMeshes){
    //     mesh.programID = programID;
    //     mesh.loadPBR("../textures/assemblies/albedo.png", "../textures/assemblies/normal.png", "../textures/assemblies/roughness.png", "../textures/assemblies/metallic.png", "../textures/assemblies/ao.png");
    // }
    ...
    // Transform translation =Transform(glm::mat3(1.0f), glm::vec3(0.0f, 80.0f, 32.0f), 1.0f);
...
void sceneNiveau2(Scene *scene){
    ...
    Transform scale = Transform().scale(0.020f);
    Transform roll = Transform().rotation(glm::vec3(1.0f, 0.0f, 0.0f), -90.0f);
    Transform yaw = Transform().rotation(glm::vec3(0.0f, 0.0f, 1.0f), 90.0f);
    Transform combined = yaw.combine_with(roll);
    ...
    if(!scene->niveau2)GOBobombBattlefieldDS.addChild(&GOBattanKing);
    ...
    if(!scene->niveau2)GOMetalMario2.collisions.push_back(&GOBattanKing);
    ...
    if(!scene->niveau2)GOBattanKing.collisions.push_back(&GOBobombBattlefieldDS);
    GOGoomba1.collisions.push_back(&GOMetalMario2);
    if(!scene->niveau2)GOBattanKing.collisions.push_back(&GOMetalMario2);
    ...
    if(!scene->niveau2)GOBattanKing.mettreAuSol(&GOBobombBattlefieldDS);
    ...
    if(!scene->niveau2)GOBattanKing.creerIA();
    if(!scene->niveau2)GOBattanKing.boiteEnglobante.setVerticesEspace(GOBattanKing.globalTransform);
    GOGoomba2.creerIA();
    GOGoomba2.boiteEnglobante.setVerticesEspace(GOGoomba2.globalTransform);
    GOGoomba3.creerIA();
    GOGoomba3.boiteEnglobante.setVerticesEspace(GOGoomba3.globalTransform);
    GOGoomba4.creerIA();
    GOGoomba4.boiteEnglobante.setVerticesEspace(GOGoomba4.globalTransform);
    GOGoomba5.creerIA();
    GOGoomba5.boiteEnglobante.setVerticesEspace(GOGoomba5.globalTransform);
    GOGoomba6.creerIA();
    GOGoomba6.boiteEnglobante.setVerticesEspace(GOGoomba6.globalTransform);
    GOGoomba7.creerIA();
    GOGoomba7.boiteEnglobante.setVerticesEspace(GOGoomba7.globalTransform);
    GOGoomba8.creerIA();
    GOGoomba8.boiteEnglobante.setVerticesEspace(GOGoomba8.globalTransform);
    // GOchainchomp.creerIA();
    // GOchainchomp.boiteEnglobante.setVerticesEspace(GOchainchomp.globalTransform);
    GOMetalMario2.pv=3;
    if(!scene->niveau2)GOBattanKing.pv=3;
    GOGoomba1.pv=1;
    GOGoomba2.pv=1;
    GOGoomba3.pv=1;
    GOGoomba4.pv=1;
...
void sceneNiveau3(Scene *scene){
    ...
    light3.setGlobalTransform(Transform(glm::mat3x3(1.0),glm::vec3(0.0,120.0,0.0),1.0));
...
std::vector<glm::vec2> verticesChargement{glm::vec2{-1.0f,1.0f},glm::vec2{-1.0f,-1.0f},glm::vec2{1.0f,-1.0f},glm::vec2{1.0f,-1.0f},glm::vec2{1.0f,1.0f},glm::vec2{-1.0f,1.0f}};
std::vector<glm::vec2> uvChargement{glm::vec2{0.0f,0.0f},glm::vec2{0.0f,1.0f},glm::vec2{1.0f,1.0f},glm::vec2{1.0f,1.0f},glm::vec2{1.0f,0.0f},glm::vec2{0.0f,0.0f}};
std::vector<unsigned short> indicesChargement{0,1,2,3,4,5};
GLuint ChargementVBuffer, ChargementIBuffer, ChargementUVBuffer, textureChargement;
void afficherChargement(){
    glGenBuffers(1, &ChargementVBuffer);
    glBindBuffer(GL_ARRAY_BUFFER, ChargementVBuffer);
    glBufferData(GL_ARRAY_BUFFER, verticesChargement.size()*sizeof(glm::vec2), &verticesChargement[0], GL_STATIC_DRAW);
    glGenBuffers(1, &ChargementIBuffer);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ChargementIBuffer);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indicesChargement), &indicesChargement[0], GL_STATIC_DRAW);
    glGenBuffers(1, &ChargementUVBuffer);
    glBindBuffer(GL_ARRAY_BUFFER, ChargementUVBuffer);
    glBufferData(GL_ARRAY_BUFFER, uvChargement.size()*sizeof(glm::vec2), &uvChargement[0], GL_STATIC_DRAW);
...
int main( void )
{
    ...
    textureChargement=loadTextureHUD("../textures/star0.png");
    ...
    do{
        ...
        glUniform1i(glGetUniformLocation(programID, "isSkybox"), GL_TRUE);
        scene.drawSkybox();
        glUniform1i(glGetUniformLocation(programID, "isSkybox"), GL_FALSE);
